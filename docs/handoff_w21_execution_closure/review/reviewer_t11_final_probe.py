                ...
                if args.reopen:
                    ...
                    for mode,levels in [('mesh',[(8,'1e-8'),(16,'1e-8'),(32,'1e-8')]),('time',[(32,'1e-3'),(32,'1e-5'),(32,'1e-7')])]:
                        ...
                        negatives={
                            ...
                            'settings_override':[dict(ref,tolerance=1e-30) for ref in refs],
                            'missing_id':[dict(refs[0],convergence_id='conv_reviewer_absent'),*refs[1:]],
                            'tampered_hash':[dict(refs[0],sha256='0'*64),*refs[1:]],
                        }
                        ...
                        for name,negative_cases in negatives.items():
                            ...
                            assert negative['execution']['dirty'] is False
                        recovered=await call('validate.convergence',{'cases':refs,'criteria':{'monotonic':True,'target_error':.1}})
                        assert recovered['data']['numerical_verification_status']=='PASS'
                        assert recovered['execution']['dirty'] is False
                        assert recovered['data']['physical_validation_status']=='UNVERIFIED'
                        summary['refinement'][mode+'_post_negative_recovery']=recovered
