...
## Preserved run composition

The recorded full63_01/full64_01 runs contain27 native candidates each and retain their FAILED terminal status from the original cache-path defect. controls63_01/controls64_01 provide the repaired cache positive/negative and restored-best solve evidence, retaining their later output-reader failure. delivery63_01/delivery64_01 provide the final successful outputs and saved MPH. Independent review explicitly composes these immutable records and checks every raw response; it does not relabel failed runs or rerun the27 candidates. The current fixed runner can execute the full recipe from source.

Final implementation source commit: `44b8e1931b47da564edb600d2127b28859d07b36`. The workpack sibling `delivery/W22_SOURCE_44b8e19.tar.gz` is a complete Git source snapshot; `delivery/SOURCE_SNAPSHOT_RECEIPT.json` records its archive SHA and per-file Git blob verification. Extract on a normal filesystem, build the wheel with the above helper, and follow the same-version commands. Final native evidence and Reviewer decisions are in the current repository alongside the snapshot. Remote publication is unconfirmed because GitHub connections were reset.
