...
ap.add_argument('--delivery-run',type=Path,help='Append final output/save evidence after a controls run stopped during output preparation.')
args=ap.parse_args()
...
controls_original_status=summary['status']
if args.delivery_run:
    delivery=json.loads((args.delivery_run/'summary.json').read_text())
    summary.update({k:v for k,v in delivery.items() if k not in ['cases','cache_repeat','source_change','changed_source_run']})
...
responses=[]
transcript_paths=[args.run/'transcript.jsonl']+([args.controls_run/'transcript.jsonl'] if args.controls_run else [])+([args.delivery_run/'transcript.jsonl'] if args.delivery_run else [])
...
if not args.cases_only:
    ...
    flag('final_delivery_terminal',summary['status'] in ['NATIVE_SMOKE_COMPLETED','SEARCH_COMPLETED'])
...
for field in ['cache_repeat','changed_source_run','final_sample','source_probes','source_probes_mm','outside_probes']:
    flag('raw_response_matches:'+field,any(payload==summary[field] for _,payload in responses))
...
result={'scope':'INDEPENDENT_FULL_SEARCH_NUMERICAL_REVIEW','review_result':'PASS' if all(c['passed'] for c in checks) else 'CHANGES_REQUIRED','native_reexecuted_by_reviewer':False,'w22_scoped_approval':False,
        ...
        'controls_original_status':controls_original_status,'appended_delivery_run':str(args.delivery_run) if args.delivery_run else None,
        'appended_delivery_transcript_sha256':hashlib.sha256((args.delivery_run/'transcript.jsonl').read_bytes()).hexdigest() if args.delivery_run else None,
