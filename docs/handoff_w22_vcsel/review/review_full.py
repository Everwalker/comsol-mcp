...
responses=[]
for line in (args.run/'transcript.jsonl').read_text().splitlines():
    row=json.loads(line)
...
flag('returned_best_is_discrete_best',summary['best_candidate']['producer']==best['producer'])
...
flag('changed_input_physical_effect',abs(changed['cases'][0]['Pabs']-cases[-1]['Pabs'])>1e-8)
