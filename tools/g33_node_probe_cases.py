...
from comsol_mcp._probe_manage import derive_history_time_axis  # noqa: E402
...
def _history_time_axis(history: Mapping[str, Any], rows: Any,
    ...
    plausible getter raises ``NoSuchMethodException``), so this helper reuses
    the production parser for the table's validated labelled column.  It also
    requires the public ``probe.history`` ``time`` and ``time_metadata`` fields
    to match that native-column parse.  COMSOL localizes the label (the
    acceptance host emits ``时间 (s)``), and a label alone never proves the
    values: the caller additionally checks that the recorded span covers the
    bound solution's stored output times (:func:`_axis_covers_stored_times`).
    ``stored_times`` is only reported here so the evidence records what the
    independent native read found.
    """
    ...
    parsed = derive_history_time_axis(headers, rows)
    evidence.update(parsed)
    if parsed.get("status") != "VERIFIED":
        evidence["public_time_metadata_match"] = False
        return [], evidence
    axis = parsed.get("values")
    public_time = history.get("time")
    public_metadata = history.get("time_metadata")
    public_values = public_metadata.get("values") if isinstance(public_metadata, Mapping) else None
    public_match = (
        isinstance(public_metadata, Mapping)
        and public_metadata.get("status") == "VERIFIED"
        and public_metadata.get("source") == "native_table_column"
        and public_values == axis
        and public_time == axis
    )
    evidence["public_time_metadata_match"] = public_match
    if not public_match:
        evidence["reason"] = "public probe.history time/time_metadata did not match the native parsed column"
        return [], evidence
    # Keep the helper's historical evidence vocabulary while recording that the
    # production parser, rather than a test-only heuristic, supplied it.
    evidence["production_source"] = parsed.get("source")
    evidence["source"] = "column"
    return (list(axis) if isinstance(axis, list) else []), evidence
