    ...
    def apply_selection(self, feature: Any) -> None:
        ...
        try:
            ...
            if named is not None:
                ...
                try:
                    # The verified COMSOL bridge accepts the documented int[] shape.
                    # A failed engine call is post-dispatch: do not retry with another
                    # overload or infer an empty selection from its exception text.
                    method(entities)
                except Exception as exc:
                    raise ExecutionContractError(
                        "SELECTION_APPLY_FAILED",
                        f"selection.set failed for entities {entities!r}: {type(exc).__name__}: {str(exc)[:500]}",
                        details={
                            "selection": list(entities),
