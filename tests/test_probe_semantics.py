...
def test_history_reads_real_complex_table_and_paginates_without_gen_result() -> None:
    ...
    assert first["headers"] == ["Time (s)", "u"]
    assert first["unit"] == "K"
    assert first["rows"][0][0] == {"real": 1.0, "imag": 0.0}
    # ``FakeTable.getTimes`` deliberately returns a different axis.  The
    # production adapter must use the labelled native table column, never a
    # guessed/nonexistent TableBaseFeature time getter.
    assert first["time"] == [1.0, 3.0]
    assert first["time_metadata"]["status"] == "VERIFIED"
    assert first["time_metadata"]["source"] == "native_table_column"
    assert first["time_metadata"]["unit"] == "s"
    assert first["time_metadata"]["scope"] == "page"
    assert first["time_metadata"]["full_row_count"] == 3
    ...
    assert second["time"] == [5.0]
    assert second["time_metadata"]["page_offset"] == 2
    assert second["next_cursor"] is None


def test_history_time_parser_accepts_localized_header_and_rejects_unverified_axes() -> None:
    parsed = probes.derive_history_time_axis(
        ["时间 (s)", "2 (1)"],
        [[{"real": 0.0, "imag": 0.0}, {"real": 2.0, "imag": 0.0}],
         [{"real": 0.5, "imag": 0.0}, {"real": 2.0, "imag": 0.0}]],
    )
    assert parsed["status"] == "VERIFIED"
    assert parsed["values"] == [0.0, 0.5]
    assert parsed["header"] == "时间 (s)"
    assert parsed["unit"] == "s"
    assert parsed["source"] == "native_table_column"

    cases = [
        (["Time (s)", "u"], [[0.0], [0.5]], "row_shape_mismatch"),
        (["Time (s)"], [[float("nan")]], "time_value_nonfinite_or_complex"),
        (["T"], [[0.0]], "no_validated_time_header"),
    ]
    for headers, rows, reason in cases:
        refused = probes.derive_history_time_axis(headers, rows)
