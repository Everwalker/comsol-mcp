...
def test_history_axis_reads_the_localized_labelled_column_of_complex_rows() -> None:
    ...
    history["time"] = [row[0]["real"] for row in history["rows"]]
    history["time_metadata"] = {
        "status": "VERIFIED", "source": "native_table_column", "header": "时间 (s)",
        "column_index": 0, "unit": "s", "values": list(history["time"]),
    }
