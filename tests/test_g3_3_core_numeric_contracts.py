...
def test_selection_set_failure_does_not_infer_empty_selection_or_retry() -> None:
    """A failed engine write is not an entity readback.

    The old fixture classified an exception containing ``out of range`` as an empty
    selection and retried the setter through a second overload.  Neither the exception
    text nor a second call proves the post-dispatch engine state, so the contract keeps
    this as an apply failure with one attempted setter call.
    """
    spec = MeasureSpec(entity_dim=1, selection={"entities": [99]})
    selection = _Selection(out_of_range=True)
    with pytest.raises(ExecutionContractError) as exc:
        spec.apply_selection(_Feature(selection))
    error = exc.value
    assert error.code == "SELECTION_APPLY_FAILED"
    assert error.stage is None  # managed dispatch must retain the post-dispatch UNKNOWN state
    assert error.details["dispatch_stage"] == "post_dispatch"
    assert "matched_entities" not in error.details
    assert selection.calls == [("set", [99])]
