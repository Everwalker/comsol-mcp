...
public final class W22Outputs {
    public static Object run(Model model, Map<String,Object> args) {
        ...
        for(String name:new String[]{"L","p0","p1","p2","ptotal","alpha","Tamb"}) {
            Map<String,Object> item=new LinkedHashMap<>();
            item.put("expression",model.param().get(name));item.put("value",model.param().evaluate(name));item.put("unit",model.param().evaluateUnit(name));
            parameterValues.put(name,item);
        }
        ...
        for(String tag:model.func().tags()) {
            ...
            try { item.put("filename",model.func(tag).getString("filename"));
            ...
            } catch(Exception ex) { item.put("read_error",ex.toString());item.put("available_properties",model.func(tag).properties()); }
            inputs.add(item);
        }
