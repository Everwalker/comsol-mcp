...
import argparse, asyncio, hashlib, json, os, shutil, sys, traceback
...
FIELDS={'T':'K','roi_mean':'K','roi_std':'K','roi_cv':'1','roi_max':'K','roi_min':'K','roi_area':'m^2',
        ...
        async with stdio_client(params) as (read,write):
            async with ClientSession(read,write) as session:
                ...
                async def call(name,body,bound=True,allow_error=False):
                    ...
                    execution=dict(binding) if bound else {}; execution.update(project_id='w22vcsel',rpc_timeout_s=3600,execution_timeout_s=None)
                ...
                if args.full:d['parameters']={'L':[.01,.02,.03],'p0':[.6,.8,1.0],'p1':[.65,.75,.85]}
                result=await call('study.sweep_manage',{'study':'std1','definition':d,'max_cases':27 if args.full else 1,'max_wall_time_s':10800})
                summary['sweep']=result;summary['cases']=result['data']['cases']
                assert all(c['status']=='COMPLETED' for c in summary['cases']), 'case failure: '+str(summary['cases'][-1])[:2000]
                ...
                summary['source_probes']=await call('result.at_points',{'spec':{'expressions':['incident','qabs'],'solution':{'dataset':'dset1'}},'points':[[x,y,.001] for x,y in spec['source_probes_m']],'coordinate_unit':'m'})
                feasible=[c for c in summary['cases'] if c['roi_mean']>=3]
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--work',required=True);p.add_argument('--version',required=True);p.add_argument('--comsol',required=True);p.add_argument('--jdk',required=True);p.add_argument('--fixture',required=True);p.add_argument('--spec',required=True);p.add_argument('--full',action='store_true')
