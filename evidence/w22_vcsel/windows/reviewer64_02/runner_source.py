...
import argparse, asyncio, hashlib, itertools, json, math, os, shutil, sys, time, traceback
from pathlib import Path
from mcp import ClientSession, StdioServerParameters
from mcp.client.stdio import stdio_client
from run_w21_native import LiveComsolServerInstance
from comsol_mcp._vcsel_source import prepare_basis, impossible_power_bound, temperature_uniformity
...
def compare_stored(actual, expected):
    """Stored temperature is unchanged; native quadrature may reorder roundoff."""
    assert actual['expressions']==expected['expressions'] and actual['solution']==expected['solution']
    errors={}
    for i,name in enumerate(actual['expressions']):
        a=actual['values'][i];b=expected['values'][i]
        def flatten(value):
            if isinstance(value,list):
                for x in value:yield from flatten(x)
            else:yield float(value)
        av=list(flatten(a));bv=list(flatten(b));assert len(av)==len(bv)
        errors[name]=max(abs(x-y) for x,y in zip(av,bv))
        assert all(math.isclose(x,y,rel_tol=1e-10,abs_tol=1e-12) for x,y in zip(av,bv)), 'stored field changed: '+name
    return errors
    ...
    try:
        ...
        async with stdio_client(params) as (read,write):
            async with ClientSession(read,write) as session:
                ...
                if args.reopen:
                    prior_root=Path(args.reopen);prior=json.loads((prior_root/'summary.json').read_text())
                    saved=prior_root/'project'/'w22_final.mph';copied=project/'w22_final.mph';shutil.copyfile(saved,copied)
                    summary['saved_sha256']=hashlib.sha256(copied.read_bytes()).hexdigest()
                    assert summary['saved_sha256']==prior['saved_sha256']
                    summary['load']=await call('model_load',{'path':str(copied)},False)
                    d=definition([prior['final_parameters']['L']],[])
                    summary['stored_before_rebind']=await call('result.at_points',d['sample'])
                    summary['before_rebind_errors']=compare_stored(summary['stored_before_rebind']['data'],prior['final_sample']['data'])
                    summary['rebind']=await call('operation_call',{'operation_id':'code.execute_java','arguments':{'source_artifact':str(source),'entrypoint':'W22Fixture','mode':'trusted','arguments':{'sources':manifest['sources'],'source_root':str(project/'source'),'reload':True}}})
                    summary['stored_after_rebind']=await call('result.at_points',d['sample'])
                    summary['after_rebind_errors']=compare_stored(summary['stored_after_rebind']['data'],prior['final_sample']['data'])
                    summary['settings']=await call('operation_call',{'operation_id':'code.execute_java','arguments':{'source_artifact':str(outputs),'entrypoint':'W22Outputs','mode':'trusted','arguments':{'plots':False}}})
                    summary['solve_count']=0;summary['status']='FRESH_WORKER_REOPEN_COMPLETED'
                else:
                    ...
                    for ordinal,(L,p0,p1) in enumerate(candidates,1):
                        ...
                        case.update(temperature_uniformity(case['roi_mean'],case['roi_std'],spec['search']['mean_deltaT_min_K']))
                    ...
                    if args.full or args.controls:
                        repeated=await call('study.sweep_manage',{'study':'std1','definition':d,'max_cases':1,'max_wall_time_s':3600})
                        summary['cache_repeat']=repeated
                        assert repeated['data']['cache_hits']==1 and repeated['data']['budget']['cases_evaluated']==0, 'same-source reuse failed'
                        selected=next(src for src in manifest['sources'] if src['L_m']==d['parameters']['L'][0] and src['group_index']==0)
                        changed=project/'source'/selected['file']; original=changed.read_bytes()
                        lines=original.decode().splitlines();middle=(len(lines)-1)//2+1;parts=lines[middle].split();parts[2]=str(float(parts[2])*1.5);lines[middle]=' '.join(parts)
                        changed.write_text('\n'.join(lines)+'\n')
                        summary['source_change']={'path':selected['file'],'before':hashlib.sha256(original).hexdigest(),'after':hashlib.sha256(changed.read_bytes()).hexdigest(),'line':middle+1}
                        try:
                            await call('operation_call',{'operation_id':'code.execute_java','arguments':{'source_artifact':str(source),'entrypoint':'W22Fixture','mode':'trusted','arguments':{'sources':manifest['sources'],'source_root':str(project/'source'),'reload':True}}})
                            changed_result=await call('study.sweep_manage',{'study':'std1','definition':d,'max_cases':1,'max_wall_time_s':3600})
                            summary['changed_source_run']=changed_result
                            assert changed_result['data']['cache_hits']==0 and changed_result['data']['budget']['cases_evaluated']==1, 'changed source reused cache'
                            assert abs(changed_result['data']['cases'][0]['Pabs']-summary['cases'][-1]['Pabs'])>1e-8, 'changed bytes did not affect actual heat source'
                        finally:
                    ...
                    final=summary['final_sample']['data']
                    summary['best_fresh_comparison']={}
                    for metric in ['roi_mean','roi_std','roi_max','roi_min','Pabs','Proi']:
                        value=final['values'][final['expressions'].index(metric)][0][4][0]
                        error=abs(value-best[metric]); limit=max(.02,.01*abs(best[metric]))
                        summary['best_fresh_comparison'][metric]={'actual':value,'best':best[metric],'error':error,'limit':limit}
                        assert error<=limit, 'fresh solution does not match chosen best: '+metric
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--work',required=True);p.add_argument('--version',required=True);p.add_argument('--comsol',required=True);p.add_argument('--jdk',required=True);p.add_argument('--fixture',required=True);p.add_argument('--spec',required=True);p.add_argument('--full',action='store_true');p.add_argument('--candidate');p.add_argument('--reopen');p.add_argument('--controls',action='store_true')
