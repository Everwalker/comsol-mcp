...
- generated: 2026-09-21T07:31:04.757846Z
- run directory: `$REPO/evidence/phase4_1/runs/20260921T072922Z-g3_1-m2b-chainB`

| case | package | status | assertions | subcases | first finding |
| --- | --- | --- | --- | --- | --- |
| W16_T019_chainB_transient | W16 | FAIL | 20 | 8 | result.sample_path returned no sample table (data keys: ['job_id', 'rpc_wait_expired', 'status']) |
...
- `FAIL` [live] normalized_max_error_le_1e-3 — result.sample_path returned no sample table (data keys: ['job_id', 'rpc_wait_expired', 'status'])
...
- `W16_T019_chainB_transient` → **IMPLEMENTATION_GAP** (classified): result.sample_path returned no sample table (data keys: ['job_id', 'rpc_wait_expired', 'status'])
