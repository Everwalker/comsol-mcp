...
public final class TableAxisInspector {
 ...
 public static Object run(Model model, Map<String,Object> args) {
  ...
  out.put("maxinner_1", safe(() -> info.getMaxInner(new int[]{1})));
  out.put("solnum_1_strict", safe(() -> info.getSolnum(1, true)));
  out.put("solnums_t", safe(() -> info.getSolnums(new String[]{"t"})));
  out.put("getVals_methods", safe(() -> {
   StringBuilder b = new StringBuilder();
   for (java.lang.reflect.Method m : info.getClass().getMethods()) {
    if (m.getName().startsWith("getVals") || m.getName().startsWith("getVal")) {
     b.append(m.getName()).append("(");
     for (Class<?> c : m.getParameterTypes()) b.append(c.getSimpleName()).append(",");
     b.append(") ");
    }
   }
   return b.toString();
  }));
  out.put("vals_1_t", safe(() -> info.getVals(1, new int[]{1})));
