...
public final class ChainABuilder {
    public static Object run(Model model, Map<String, Object> args) {
        model.modelNode().create("body");
        model.geom().create("lineg", 1);
        model.geom("lineg").create("r1", "Interval");
        model.geom("lineg").feature("r1").set("p", "0 2");
        model.geom("lineg").run();
        
        model.physics().create("ht", "HeatTransfer", "lineg");
        ...
        model.physics("ht").feature("temp2").selection().set(new int[]{2});
        model.physics("ht").feature("temp2").set("T0", "353.15[K]");
        
        model.material().create("mat1", "Common", "body");
