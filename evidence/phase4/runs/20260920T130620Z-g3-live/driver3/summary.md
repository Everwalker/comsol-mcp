...
- generated: 2026-09-20T14:16:04.443389Z
- run directory: `$REPO/evidence/phase4/runs/20260920T130620Z-g3-live/driver3`

| case | package | status | assertions | subcases | first finding |
| --- | --- | --- | --- | --- | --- |
| R01_LIVE | R | FAIL | 11 | 8 | an unsupported expression/unit write was not rejected before the engine write |
| R03_LIVE | R | BLOCKED | 9 | 10 | static preview did not reject an empty index array / missing entry key |
| R04_LIVE | R | BLOCKED | 11 | 9 | the preview rejected the probes with codes outside the documented vocabulary (EXECUTION_STATE_UNKNOWN, EXECUTION_STATE_UNKNOWN), so the R04 invariant contract c |
| R_READBACK | R | FAIL | 2 | 4 | roll-up of R01_LIVE: FAIL |
| W13_T006_variables | W13 | BLOCKED | 6 | 7 | variable.group_create returned reconcile unfinished engine work before new operations |
| W13_T015_units | W13 | BLOCKED | 5 | 6 | physics.create returned reconcile unfinished engine work before new operations |
| W13_T048_selection_drift | W13 | BLOCKED | 5 | 6 | selection.create returned reconcile unfinished engine work before new operations |
| W13_T016_2D_data | W13 | BLOCKED | 8 | 6 | function.create returned reconcile unfinished engine work before new operations |
| W14_T009_geometry_edit | W14 | BLOCKED | 5 | 7 | geometry.feature_create returned reconcile unfinished engine work before new operations |
| W14_T034_local_paths | W14 | BLOCKED | 9 | 5 | model.save returned reconcile unfinished engine work before new operations |
| W15_T007_selections | W15 | BLOCKED | 5 | 6 | physics.selection_set returned reconcile unfinished engine work before new operations |
| W15_T017_material | W15 | BLOCKED | 6 | 5 | the material property write was refused: the engine state was unknown (reconcile unfinished engine work before new operations) |
| W15_T042_license | W15 | BLOCKED | 7 | 5 | BLOCKED_LICENSE: EXECUTION_STATE_UNKNOWN |
| W16_T018_mesh | W16 | BLOCKED | 6 | 7 | mesh.create returned reconcile unfinished engine work before new operations |
| W16_T019_chainA_steady | W16 | BLOCKED | 7 | 12 | model.create returned EXECUTION_STATE_UNKNOWN |
| W16_T019_chainB_transient | W16 | BLOCKED | 6 | 6 | model.create returned EXECUTION_STATE_UNKNOWN |
| W16_T019_chainC_continue | W16 | BLOCKED | 7 | 7 | model.load returned reconcile unfinished engine work before new operations |
| W16_T020_solver | W16 | BLOCKED | 9 | 8 | study.list returned reconcile unfinished engine work before new operations |
| GUARD_T010 | GUARD | NOT_RUN | 7 | 5 |  |
| GUARD_T035 | GUARD | BLOCKED | 10 | 7 | an outside-workspace documentation source was accepted or the denial was not explicit: the call was refused with an unknown engine state (EXECUTION_STATE_UNKNOW |
| GUARD_T038 | GUARD | PASS | 10 | 7 |  |
| GUARD_T005 | GUARD | PASS | 6 | 4 |  |
| GUARD_T033 | GUARD | FAIL | 7 | 4 | operation_describe does not publish an evaluation policy for evaluate_expressions in this build (no isolation or node-ownership contract), so the T033 policy co |

cases: PASS 2 / FAIL 3 / BLOCKED 17 / NOT_RUN 1
...
- `FAIL` [live] incompatible_kind_or_unit_rejected_before_write — an unsupported expression/unit write was not rejected before the engine write
- `BLOCKED` [live] non_matching_readback_not_silently_accepted
- `FAIL` [live] nonfinite_text_strictly_rejected — non-finite numeric text was not rejected with the value left unchanged

### R03_LIVE — BLOCKED
- `PASS` [static] static_indexed_ops_executable
- `BLOCKED` [protocol] static_malformed_index_and_key_rejected_pre_engine — static preview did not reject an empty index array / missing entry key
...
- `BLOCKED` [protocol] static_unsupported_invariant_rejected_pre_write — the preview rejected the probes with codes outside the documented vocabulary (EXECUTION_STATE_UNKNOWN, EXECUTION_STATE_UNKNOWN), so the R04 invariant contract could not be classified
- `PASS` [protocol] root_node_read_refusal_is_structured
- `BLOCKED` [live] invariant_violation_not_reported_as_pass — the transaction reported an unknown engine state (EXECUTION_STATE_UNKNOWN); no verification status was established
- `BLOCKED` [live] verified_transaction_status_split — a satisfied transaction did not report execution and verification status separately: the engine state was unknown (reconcile unfinished engine work before new operations)
- `BLOCKED` [live] stale_revision_rejected — a stale expected_revision was not rejected: the engine state was unknown (reconcile unfinished engine work before new operations)
...
### R_READBACK — FAIL
- `FAIL` [rollup] r01_expression_readback_status — roll-up of R01_LIVE: FAIL
...
- `BLOCKED` [live] variables_two_in_one_group — variable.group_create returned reconcile unfinished engine work before new operations
...
- `BLOCKED` [live] surface_source_W_per_m2 — physics.create returned reconcile unfinished engine work before new operations
...
- `BLOCKED` [live] named_selection_created_and_bound — selection.create returned reconcile unfinished engine work before new operations
...
- `BLOCKED` [live] two_d_interpolation_angular_difference_preserved — function.create returned reconcile unfinished engine work before new operations
...
- `BLOCKED` [live] work_plane_and_array_located — geometry.feature_create returned reconcile unfinished engine work before new operations
...
- `BLOCKED` [live] model_save_load_roundtrip_hash — model.save returned reconcile unfinished engine work before new operations
...
- `BLOCKED` [live] physics_level_selection_set — physics.selection_set returned reconcile unfinished engine work before new operations
...
- `BLOCKED` [live] k_T_Cp_T_and_rho_expressions — the material property write was refused: the engine state was unknown (reconcile unfinished engine work before new operations)
...
- `BLOCKED` [live] free_tet_sequence_created — mesh.create returned reconcile unfinished engine work before new operations
...
- `BLOCKED` [live] target_only_modified — model.load returned reconcile unfinished engine work before new operations
- `NOT_RUN` [live] non_target_nodes_preserved — prerequisite subcase target_only_modified did not pass (EXECUTION_STATE_UNKNOWN)
- `NOT_RUN` [live] manual_solver_preserved — prerequisite subcase target_only_modified did not pass (EXECUTION_STATE_UNKNOWN)
- `NOT_RUN` [live] derived_values_and_data_association_preserved — prerequisite subcase target_only_modified did not pass (EXECUTION_STATE_UNKNOWN)
- `NOT_RUN` [live] solve_after_continuation — prerequisite subcase target_only_modified did not pass (EXECUTION_STATE_UNKNOWN)

### W16_T020_solver — BLOCKED
- `PASS` [static] static_solver_ops_availability
- `BLOCKED` [live] study_target_read — study.list returned reconcile unfinished engine work before new operations
...
- `BLOCKED` [live] outside_workspace_source_denied — an outside-workspace documentation source was accepted or the denial was not explicit: the call was refused with an unknown engine state (EXECUTION_STATE_UNKNOWN; released through job_reconcile)
- `BLOCKED` [live] symlink_outside_denied — a symlink pointing outside the workspace was accepted as a documentation source: the call was refused with an unknown engine state (EXECUTION_STATE_UNKNOWN; released through job_reconcile)
- `BLOCKED` [live] outside_write_attempt_denied — an outside directory could be registered as an indexable source: the call was refused with an unknown engine state (EXECUTION_STATE_UNKNOWN; released through job_reconcile)
