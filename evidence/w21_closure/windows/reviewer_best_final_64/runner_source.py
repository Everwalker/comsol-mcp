        ...
        async with stdio_client(params) as (read,write):
            async with ClientSession(read,write) as session:
                ...
                if args.reopen:
                    ...
                    await call('model_create',{'name':'ReviewerBestIndependent'},bound=False)
                    await call('operation_call',{'operation_id':'code.execute_java','arguments':{'source_artifact':str(source),'entrypoint':'W21Fixture','mode':'trusted','arguments':{}}})
                    sample={'spec':{'expressions':['T'],'solution':{'dataset':'dset1'}},'points':[[.0125,.005,.005],[.025,.005,.005],[.0375,.005,.005]],'coordinate_unit':'m'}
                    definition={'parameters':{'k':[390.0],'rhoCp':[3400000.0]},'units':{'k':'W/(m*K)','rhoCp':'J/(m^3*K)'},'sample':sample,'metrics':{'T_mid':{'expression':'T','unit':'K','indices':[0,4,1]},'objective':{'expression':'T','unit':'K','indices':[0,4,1],'target':304}},'times':[0,.5,1,1.5,2],'validation':{'range':[299.9,310.1]}}
                    result=await call('study.sweep_manage',{'study':'std1','definition':definition,'max_cases':1})
                    case=result['data']['cases'][0]
                    assert case['status']=='COMPLETED' and case['parameters']=={'k':390.0,'rhoCp':3400000.0}
                    values=case['sample']['values'][0][0];times=case['solution_indices']['time_values']
                    errors=[abs(values[i][j]-(300+10*math.sin(math.pi*x/.05)*math.exp(-390/3400000*(math.pi/.05)**2*t))) for i,t in enumerate(times) for j,x in enumerate([.0125,.025,.0375])]
                    assert max(errors)<=.03
                    midpoint=values[4][1];objective=abs(midpoint-304)
                    assert 300<=midpoint<=310
                    summary['independent_best']={'result':result,'max_error_K':max(errors),'T_mid_K':midpoint,'objective_K':objective,'constraint_pass':True}
                    summary['save']=await call('save_model',{'path':str(project/'reviewer_best.mph')})
                    summary['saved_sha256']=hashlib.sha256((project/'reviewer_best.mph').read_bytes()).hexdigest()
                    summary['status']='SMOKE_COMPLETED'
                else:
